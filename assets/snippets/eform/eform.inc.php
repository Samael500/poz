...
## ƒл€ того, чтобы можно было использовать продолжать eForm при отправке писем с сайта необходимо произвести следующие манипул€ции:
## в каталоге ../assets/snipets/eform находим файл eform.inc.php. ≈го необходимо открыть каким нибудь простым текстовым редактором.
## ƒалее, в этом файле необходимо найти все строки "$mail->IsMail();" и комментиру€ каждую такую строку, добавить непосредственно ниже нее:
...
##	smtp::hostinger.com.ua
##	$mail->IsSMTP(); // отсылать использу€ SMTP
##	$mail->Host	 = "mx1.hostinger.com.ua"; // SMTP сервер
##	$mail->Port = 2525; // SMTP port
##	$mail->SMTPAuth = true;	 // включить SMTP аутентификацию
##	$mail->Username = "admin@poz.pp.ua"; // SMTP username
##	$mail->Password = "RKnipirFjQWxMYQG"; // SMTP password

## где вместо mail.life-host.net необходимо указать выданный вам бесплатным хостингом SMTP сервер; вместо user@domain.com ваш логин SMTP, а вместо pass -- пароль.
